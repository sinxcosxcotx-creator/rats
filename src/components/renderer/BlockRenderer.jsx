...
//
// 说明：
// - 现在血线内容已迁移到 content/**/bloodlines/*.json，
//   因此 bloodline-grid 不再依赖 @data/bloodlines.json。
...
// 示例数据：待领养仍可由 data 覆盖（你要完全迁走也可以再改）
...
const BlockRenderer = ({ block }) => {
  ...
  switch (type) {
    ...
    // 7) 血线网格（不再依赖 @data/bloodlines.json）
    case 'bloodline-grid': {
      ...
      if (items.length === 0) {
        return (
          <div className="text-sm text-white/50">
            bloodline-grid: no items provided. Please set <code>items</code> in your JSON.
          </div>
        );
      }
